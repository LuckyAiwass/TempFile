...
public class OTGActivity extends Activity {
        ...
        public void handleMessage(Message msg) {
			...
			ULog.i("apkfile path: "+"/storage/emulated/0/installapps/"+packageName+".apk");
			    ...
			    ULog.i("delete the apk in installapp");
                    ...
                    //Toast.makeText(OTGActivity.this, packageName+" "+ PackageUtil.installStatusToString(status), Toast.LENGTH_SHORT).show();
                   ...
                   /* switch (status) {
                        case PackageInstaller.STATUS_SUCCESS:
                            //Toast.makeText(OTGActivity.this, getString(R.string.installcomp), Toast.LENGTH_SHORT).show();
                            if(!portinstall){
                            ...
                            break;
                        default:
                            //Toast.makeText(OTGActivity.this, getString(R.string.installerror), Toast.LENGTH_SHORT).show();
                            if(!portinstall){
                                progressLayout.setVisibility(View.GONE);
                                mainLayout.setVisibility(View.VISIBLE);
                            }else{
	...
	mPackageManager = getPackageManager();
        ...
        reScan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                // TODO Auto-generated method stub
                installed = 0;
                checkedApk = SharedPrefsStrListUtil.getStrListValue(OTGActivity.this, "ADD_APK");
                if(checkedApk.size()==0){
                     Toast.makeText(OTGActivity.this, "Please select the file", Toast.LENGTH_SHORT).show();
                }else{
    ...
    public void updateData() {
        ...
        String udistpath = getUdistPath(OTGActivity.this);
        if (udistpath.equals("")){
            udistpath = getSDPath(OTGActivity.this);
	}
        //如果还是空finish()
        ...
        ULog.d("udistpath :" + udistpath);
        File file = new File(udistpath);
	...
	File apkFile = new File(apkFilePath);
        ...
        if(info != null){
            ...
            List<String> list = getcustomizeInstallApps();
            ULog.i("otg installAPK getcustomizeInstallApps list:"+list);
            if(list != null && list.size() > 0){
                if(list.contains(pkgName)){
                    try{
                        ULog.d("handleStartCopy:"+pkgName);
                        copyFile(apkFile, new File("/customize/app/"+pkgName+".apk"));
                    } catch(IOException e) {
                        e.printStackTrace();
                    }
                }
	...
	public void onPackageInstalled(String packageName, int returnCode, String msgStr, Bundle extras) {
		...
		ULog.i("checkedApk.size(): "+checkedApk.size());
                SharedPrefsStrListUtil.removeStrListItem(OTGActivity.this, "ADD_APK",new File(checkedApk.get(0)).getAbsolutePath());
                checkedApk.remove(0);
    ...
    void onPackageInstalled(int statusCode) {
        ...
        Toast.makeText(OTGActivity.this, "INSTALL FAILURE", Toast.LENGTH_SHORT).show();
    }
            ...
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                   ...
                   /* if(holder.enabled.isChecked()){
                        ...
                        ULog.d("addStrListValue , list.get(position).getAbsolutePath()");
                       // choiceApk.add(position,true);
                        SharedPrefsStrListUtil.addStrListValue(context, "ADD_APK",list.get(position).getAbsolutePath());
                    }else{
ULog.d("removeStrListItem , list.get(position).getAbsolutePath()");
