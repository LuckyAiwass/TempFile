 ...
 */
package com.ubx.factorykit.LightSensor;
...
public class LightSensorService extends Service {
    ...
    private final static String TAG = "LightSensorService";
    private static boolean result = false;
    private static Context mContext = null;
    private static int index = -1;
    private static SensorManager LightSensorManager = null;
    ...
    private void start() {
        ...
        if (WORKROUND) {
            ...
            if (!LightSensorManager.registerListener(mLightSensorListener, mLightSensor, SENSOR_DELAY)) {
    ...
    public static void finish(boolean writeResult) {
        ...
        if (!WORKROUND)
            try {
                LightSensorManager.unregisterListener(mLightSensorListener, mLightSensor);
            } catch (Exception e) {
            }
