...
package com.ubx.factorykit.PSensor;
...
public class PSensor extends Activity {
    ...
    void getService() {
        ...
        if (!mSensorManager.registerListener(mPSensorListener, mPSensor, SensorManager.SENSOR_DELAY_FASTEST)) {
    ...
    void updateStatusView(int status, String psData) {
        ...
        case STAUS_COVERED:
            ...
            mImageTextView.setText("Status" + " : " + getString(R.string.psensor_covered));
            break;
        case STAUS_INIT:
        case STAUS_UNCOVERED:
            mImageTextView.setImageResource(R.drawable.off);
            mImageTextView.setText("Status" + " : " + getString(R.string.psensor_uncovered));
            break;
        case STAUS_CHANGING:
            mImageTextView.setImageResource(R.drawable.changing);
            mImageTextView.setText("Status" + " : " + getString(R.string.psensor_verifying));
            break;
        default:
            break;
        }
        ...
        public void onSensorChanged(SensorEvent event) {
            ...
            synchronized (this) {
                if (event.sensor.getType() == SENSOR_TYPE) {
                    ...
                    if (initFlag) {
                        String ps = Utilities.readFile(PS_FILE);
                        updateStatusView(status, ps);
                        logd(mCommandCoverTimer.isRunning + " " + status + " " + command);
                        if (mCommandCoverTimer.isRunning || mCommandUncoverTimer.isRunning)
    ...
    protected void onDestroy() {
        ...
        if (mSensorManager == null || mPSensorListener == null || mPSensor == null)
            return;
        mSensorManager.unregisterListener(mPSensorListener, mPSensor);
    }
