package com.ubx.factorykit.CameraFront;
...
//        if (!singleCamera) {
            ...
            for (int cameraId = 0; cameraId < numberOfCameras; cameraId++) {
                ...
                if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_BACK /*&& !hasBack*/) {
                    ...
                    if (hasFront && cameraId == numberOfCameras - 1)
                        continue;
                    // urovo huangjiezhuo add end
                    frontCameraId = cameraId;
                    Log.v("zgy","front id:" + frontCameraId);
                    hasFront = true;
                }
            }
//        }
    ...
    private void initCamera(boolean isBack) {
        try {
            //urovo huangjiezhou modify on 20211028
            int cameraId = isBack ? backCameraId : frontCameraId;
    ...
    // urovo huangjiezhuo add begin on 20211206
    public static boolean checkCameraFront() {
        int CameraNumMax = Camera.getNumberOfCameras();// 获取摄像头个数
        Log.v("zgy","numberOfCameras:" +CameraNumMax);
        for (int cameraId = 0; cameraId < CameraNumMax; cameraId++) {
            Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
            Camera.getCameraInfo(cameraId, cameraInfo);
