...
import com.ubx.factorykit.Framework.MainApp.ServiceInfo;
...
public class AutoService extends IntentService {
    ...
    CountDownTimer taskCountDownTimer = new CountDownTimer(FINISH_TIME, INTERVAL_TIME) {
        ...
        public void onTick(long arg0) {
            ArrayList<MainApp.ServiceInfo> intentArrayList = MainApp.getInstance().intentArrayList;
    ...
    private void init(Context context) {
        ...
        // yangkun modify start 2020/03/12
        //if (!Build.PWV_CUSTOM_CUSTOM.equals("YINGFENG")) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if ("true".equals(item.get("auto")))
                if (Utilities.RESULT_FAIL.equals(item.get("result")) || "NULL".equals(item.get("result"))) {
                    ...
                    MainApp.getInstance().addServiceList(new MainApp.ServiceInfo(serviceIntent, item.get("parameter")));
                }
        }
