...
static const char USER_BOOTANIMATION_FILE[] ="/mnt/vendor/persist/media/bootanimation.zip"; //urovo add for UFS
...
void BootAnimation::findBootAnimationFile() {
    ...
    // urovo modify for UFS begin
    //static const bool playDarkAnim = android::base::GetIntProperty("ro.boot.theme", 0) == 1;
    static const char* bootFiles[] =
        {APEX_BOOTANIMATION_FILE, //*playDarkAnim ? PRODUCT_BOOTANIMATION_DARK_FILE : */PRODUCT_BOOTANIMATION_FILE,
         OEM_BOOTANIMATION_FILE, USER_BOOTANIMATION_FILE,SYSTEM_BOOTANIMATION_FILE};
    
    static const char* cusbootFiles[] ={USER_BOOTANIMATION_FILE,PRODUCT_BOOTANIMATION_FILE, OEM_BOOTANIMATION_FILE,SYSTEM_BOOTANIMATION_FILE};
    static const char* shutdownFiles[] =
        {PRODUCT_SHUTDOWNANIMATION_FILE, OEM_SHUTDOWNANIMATION_FILE, SYSTEM_SHUTDOWNANIMATION_FILE, ""};

    FILE *fp = NULL;
    char buff[8]={"0"};
    fp = fopen("/mnt/vendor/persist/media/bootanimationstate.txt", "r");
    if(fp != NULL){
         fgets(buff, 2, (FILE*)fp);
    }else{
        ALOGE("Can't find　/mnt/vendor/persist/media/bootanimationstate.txt ");
    }
    const char* fufs="1";
    ALOGE("cat　/mnt/vendor/persist/media/bootanimationstate.txt:%s", buff);
    ALOGE("strcmp(fufs,buff):%d", strcmp(fufs,buff));
    if(strcmp(fufs,buff)){//==返回0
        for (const char* f : (!mShuttingDown ? cusbootFiles  : shutdownFiles)) {
           if (access(f, R_OK) == 0) {
                mZipFileName = f;
                ALOGE("cat　q1 mZipFileName:%s", f);
                return;
            }
        }
    } else {
