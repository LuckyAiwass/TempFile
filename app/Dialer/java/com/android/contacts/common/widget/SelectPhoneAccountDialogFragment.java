  ...
  static class SelectAccountListAdapter
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
      ...
      if (account == null) {
	   ...
	   String simOperate =account.getLabel().toString(); 
      if (!TextUtils.isEmpty(simOperate)) {
	   if("zh".equals(Locale.getDefault().getLanguage())){
	      if(simOperate.equals("CMCC")){
	         simOperate= getContext().getString(R.string.china_mobile);
		  }else if(simOperate.equals("China Unicom")){
			  simOperate= getContext().getString(R.string.china_Unicom);
		  }
	   }else if("en".equals((Locale.getDefault().getLanguage()))){
		   if(simOperate.equals("中国电信")){			   
			   simOperate= getContext().getString(R.string.china_Telecom);
		   	}

	   }
      }
      holder.labelTextView.setText(simOperate);
   /*wanghongjian add change string*/ 
