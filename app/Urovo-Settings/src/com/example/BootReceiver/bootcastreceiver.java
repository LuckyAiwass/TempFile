...
public class bootcastreceiver extends BroadcastReceiver{
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if(intent.getAction().equals("android.intent.action.nfc.properties")){
            ...
            String ufsstatus = android.os.SystemProperties.get("ro.ufs.build.date.utc","0"); //prop from UFS 
            String ufsState = android.os.SystemProperties.get("persist.sys.ufs.status","1"); //prop determine whether to parse UFS
            Log.i("urovo","ufsstatus =======" + ufsstatus + "          ufsState============"+ufsState);
            if(!ufsstatus.equals(ufsState)){
                try {
                    Log.d("urovo","start ufs =========");
