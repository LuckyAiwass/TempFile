...
package android.signaturepad.views;
...
import android.signaturepad.utils.Bezier;
import android.signaturepad.utils.ControlTimedPoints;
import android.signaturepad.utils.JBIGEncoder;
import android.signaturepad.utils.TimedPoint;
...
public class SignaturePad extends View {
    ...
    private static  int DEFAULT_SCALE_WIDTH = 237;
    private static  int DEFAULT_SCALE_HEIGHT = 79;
    public static  int SIGNATUREPAD_WIDTH = 320;
    public static  int SIGNATUREPAD_HEIGHT = 107;
    public SignaturePad(Context context, AttributeSet attrs) {
        super(context, attrs);

        /*TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SignaturePad,
                0, 0);*/
        Resources resources = context.getResources();
        scale = resources.getDisplayMetrics().density;
        DEFAULT_SCALE_WIDTH = (int)dip2px(context, SIGNATUREPAD_WIDTH);
        DEFAULT_SCALE_HEIGHT = (int)dip2px(context, SIGNATUREPAD_HEIGHT);
        // Configurable parameters
        /*try {
    ...
    public void setOverlayInfo(String overlayText) {
        overlayInfo = overlayText;
        setSignatureOverlayInfo();
    }
    ...
    public int getJBIGSignature(int scaleWidth, int scaleHeight, byte[] resJBIG) {
        if(scaleWidth <= 0) scaleWidth = DEFAULT_SCALE_WIDTH;
        if(scaleHeight <= 0) scaleHeight = DEFAULT_SCALE_HEIGHT;
        Bitmap originalBitmap = getTransparentSignatureBitmap();
        Bitmap whiteBgBitmap = Bitmap.createBitmap( scaleWidth, scaleHeight, Bitmap.Config.ARGB_8888);
        ...
        JBIGEncoder jbigEncoder = new JBIGEncoder();
        int ret = jbigEncoder.JBIGCompressing(whiteBgBitmap, resJBIG);
        return ret;
    }
    ...
    private void setSignatureOverlayInfo() {
            if (mSignatureBitmap == null) {
                mSignatureBitmap = Bitmap
                        .createBitmap(DEFAULT_SCALE_WIDTH, DEFAULT_SCALE_HEIGHT, Bitmap.Config.ARGB_8888);
            ...
            paint.getTextBounds(overlayInfo, 0, overlayInfo.length(), bounds);
            // draw text to the bottom
            int x = (mSignatureBitmap.getWidth() - bounds.width())/2;
            int y = (mSignatureBitmap.getHeight() - bounds.height())/2 + bounds.height();
            canvas.drawText(overlayInfo, x, y, paint);
            invalidate();
    }
    ...
    public Bitmap getTransparentSignatureBitmap(boolean trimBlankSpace) {
        ...
        for (int i = 0; i < imgHeight; i++) {
            for (int j = imgWidth - 1; j >= 0; j--) {
                if (mSignatureBitmap.getPixel(j, i) != Color.TRANSPARENT && j > width) {
                    width = j;
                    break;
                }
            }
        ...
        for (int i = 0; i < imgWidth; i++) {
            for (int j = imgHeight - 1; j >= 0; j--) {
                if (mSignatureBitmap.getPixel(i, j) != Color.TRANSPARENT && j > height) {
                    height = j;
                    break;
                }
            }
    ...
    private void addPoint(TimedPoint newPoint) {
        ...
        if (mPoints.size() > 2) {
            ...
            ControlTimedPoints tmp = calculateCurveControlPoints(mPoints.get(0), mPoints.get(1),
                    mPoints.get(2));
            ...
            velocity = mVelocityFilterWeight * velocity + (1 - mVelocityFilterWeight)
                    * mLastVelocity;
    ...
    private ControlTimedPoints calculateCurveControlPoints(TimedPoint s1, TimedPoint s2,
            TimedPoint s3) {
        float dx1 = s1.x - s2.x;
        float dy1 = s1.y - s2.y;
        float dx2 = s2.x - s3.x;
        ...
        return new ControlTimedPoints(new TimedPoint(m1.x + tx, m1.y + ty), new TimedPoint(m2.x
                + tx, m2.y + ty));
    }
