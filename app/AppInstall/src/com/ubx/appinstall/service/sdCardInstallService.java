...
public class sdCardInstallService extends IntentService {
    ...
    private Handler mInstallHandler = new MyHandler(sdCardInstallService.this);
    private Handler mBootHandler = new BootHandler(sdCardInstallService.this);
    private Handler mHandler = new ReturnHandler(sdCardInstallService.this);
    private boolean powerboot = false;
    private int i = 0;
    private ImageView loadingImg;
    private int installedNum = 0;
    private Animation animation;
    private int errorInstall = 0;
    private StringBuilder installResult = new StringBuilder();

    private static class MyHandler extends Handler {
        private WeakReference<sdCardInstallService> mReference = null;

        public MyHandler(sdCardInstallService context) {
            mReference = new WeakReference<sdCardInstallService>(context);
        }

        @Override
        public void handleMessage(Message msg) {
            sdCardInstallService context = mReference.get();
    ...
    //有返回值安装
    private static class ReturnHandler extends Handler {

        private WeakReference<sdCardInstallService> mReference = null;

        public ReturnHandler(sdCardInstallService context) {
            mReference = new WeakReference<sdCardInstallService>(context);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            sdCardInstallService context = mReference.get();
            context.installedNum++;
            int current = (int) ((context.installedNum / (float) context.installApkList.size()) * 100);
            ULog.d("current :" + current);
            if (context.runback && context.mRemoteViews != null) {
                context.mRemoteViews.setTextViewText(R.id.install_notification_progress_text, "" + current + "%");
                context.mRemoteViews.setProgressBar(R.id.install_notification_progress_bar, 100, current, false);
                context.notifyInstallNotification();
            ...
            switch (msg.what) {
                case INSTALL_COMPLETE:
                    int status = msg.arg1;
                    ULog.d("handleMessage , status:" + PackageManager.installStatusToString(status));
                    switch (status) {
                        case PackageManager.INSTALL_SUCCEEDED:
                            ((TextView) context.installView.findViewById(R.id.target_Text))
                                    .setText((String) msg.obj == null ? "" : (String) msg.obj +" "+ PackageManager.installStatusToString(status));
                            context.installResult.append((String) msg.obj + "\t\t\t" + PackageManager.installStatusToString(status)+"\n");
                            break;
                        default:
                            ((TextView) context.installView.findViewById(R.id.target_Text))
                                    .setText((String) msg.obj == null ? "" : (String) msg.obj + PackageManager.installStatusToString(status));
                            context.showToast((String) msg.obj +" "+ PackageManager.installStatusToString(status));
                            context.errorInstall++;
                            context.installResult.append((String) msg.obj + "\t\t\t" + PackageManager.installStatusToString(status)+"\n");
                            break;
                    }
                    if (context.installedNum == context.count) {
                        if (context.runback) {
                            context.cancelInstallNotification();
                        } else {
                            context.installAlerDialogDismiss();
                        }
                        String status_Str =  context.errorInstall == 0? "Install complete":"Install complete "+ context.errorInstall+" Fail";
                        context.showToast(status_Str);
                        context.showDialog(context.installResult.toString());
                    }
                    break;
    ...
    public static class BootHandler extends Handler {
        ...
        public BootHandler(sdCardInstallService context) {
            mReference = new WeakReference<sdCardInstallService>(context);
        }
    ...
    public sdCardInstallService() {
        super("sdCardInstallService");
        // TODO Auto-generated constructor stub
        ULog.d("sdCardInstallService");
    ...
    public void onCreate() {
        // TODO Auto-generated method stub
        super.onCreate();
        installResult.append("PackageName\t\t\tStatus\n\n");
    ...
    private void installAlertDialog() {
        ...
        loadingImg = (ImageView) installView.findViewById(R.id.loadingImg);
                ...
                if (f != null && f.exists()) {
                    if (f.isDirectory()) {
                        ...
                        if (!isInstalled(apkName, apkVersion)) {
                            ULog.d("installApkList.add apk " + apkName);
    ...
    public boolean installApps(File file) {
        ...
        //}
        List<String> selectedAPK = SharedPrefsStrListUtil.getStrListValue(getApplicationContext(), "ADD_APK");
        if (selectedAPK.size() != 0) {
            for (String path : selectedAPK) {
                ULog.d("selectedAPK path = " + path);
                File addapk = new File(path);
                if (addapk.isFile() && addapk.getAbsolutePath().toLowerCase().endsWith(".apk"))
                    installApkList.add(addapk);
            }
        }
    ...
    private boolean installAPK(File apkFile) {
        ...
        ULog.d("------installAPK apkFile " + apkFile.getPath());
