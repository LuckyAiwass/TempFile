...
public class SharedPrefsStrListUtil {
	...
	/** 数据存储的XML名称 **/
	public final static String SETTING = "AutoInstall";
	...
	public static void addStrListValue(Context context,String key,String value){
		if(null == value||value.equals("")){
			return ;
		}
		int size = getIntValue(context,key + "size",0);
		List <String> list  = getStrListValue(context,key);
		for(String path : list){
			if(path.equals(value)){
				return;
			}
		}
		putStringValue(context,key + size ,value);
		putIntValue(context,key + "size" , ++size);
		ULog.i("SP  add success ---------------->" + value);
	}
	/**
	 * 取出数据（int)
	 * 
