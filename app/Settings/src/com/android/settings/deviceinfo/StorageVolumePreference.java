...
public class StorageVolumePreference extends Preference {
    ...
    public StorageVolumePreference(Context context, VolumeInfo volume, long totalBytes) {
        ...
        if (volume.isMountedReadable()) {
            ...
            long freeBytes = 0;
            long usedBytes = 0;
			StatFs fs = new StatFs(Environment.getDataDirectory().getPath());//songtingting modify
            freeBytes = (VolumeInfo.ID_PRIVATE_INTERNAL.equals(volume.getId())) ? fs.getAvailableBytes() : path.getFreeSpace();
            if (volume.getType() == VolumeInfo.TYPE_PRIVATE) {
                ...
                //freeBytes = path.getFreeSpace(); songtingting modify
				usedBytes = totalBytes - freeBytes;
            }
