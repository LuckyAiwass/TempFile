...
public class PhoneNumberPreferenceController extends BasePreferenceController {
    ...
    public void displayPreference(PreferenceScreen screen) {
        ...
        for (int simSlotNumber = 1; simSlotNumber < mTelephonyManager.getPhoneCount();
            ...
            multiSimPreference.setSelectable(false);//songtingting modify phone number preference not selectable 20210907
            screen.addPreference(multiSimPreference);
            mPreferenceList.add(multiSimPreference);

            // yangkun add start 2021/11/05
            String strSupport = SystemProperties.get("persist.sys.urv.wifionly_enable", "false");
            if (strSupport.equals("true")) {
                screen.removePreference(multiSimPreference);
            }
            // add end
        }
    }
