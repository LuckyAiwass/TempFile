...
class UpdateParser {
	...
	private static final String SPLASH_BIN_FILE = "splash";
	private static final String CUSTOMIZE_BIN_FILE = "customize";
	public static final int ONLY_UFS_PACKAGE = 1;
	public static final int ONLY_AB_PACKAGE = 2;
	public static final int AB_UFS_PACKAGE = 3;
	public static final int UNKNOW_PACKAGE = 4;
	//add end
	...
	//add by chenchuanliang for UFS update 20200711
    @Nullable
    static int getPackageStatus(@NonNull File file) throws IOException {
        Preconditions.checkNotNull(file);

        boolean payloadFound = false;
		boolean ufsFound = false;
        ...
        try (ZipFile zipFile = new ZipFile(file)) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (entry.isDirectory()) {
                    ...
                    payloadFound = true;
                } else if (entry.getName().contains(SPLASH_BIN_FILE)) {
                    ufsFound = true;
                } else if (entry.getName().contains(CUSTOMIZE_BIN_FILE)) {
                    ufsFound = true;
                }
            }
			if (payloadFound&&ufsFound) 
				return AB_UFS_PACKAGE;
			if (!payloadFound&&ufsFound) 
				return ONLY_UFS_PACKAGE;
			if (payloadFound&&!ufsFound) 
				return ONLY_AB_PACKAGE;
        }
		return UNKNOW_PACKAGE;
    }
	//add end
    /**
