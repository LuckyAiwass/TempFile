...
public class SettingsActivity extends SettingsBaseActivity
	...
	private int mFrom=0;  //wangchunyan 20190417
	private int mFromContacts=0;  //chengyuwen 20190419
    ...
    protected void onCreate(Bundle savedState) {
		...
		//[S]wangchunyan 20190417 add
		mFrom=intent.getIntExtra("fromgallery",0);
		mFromContacts=intent.getIntExtra("fromContacts",0);//chengyuwen
		//[E]wangchunyan 20190417 add
        ...
        if (actionBar != null) {
			//[S]wangchunyan 20190418 add to fix bug 53888
			Log.i("SettingsActivity", "[wcy] onCreate mFrom:"+mFrom +",deviceProvisioned="+deviceProvisioned);
			if(mFrom!=0||mFromContacts==1)//chengyuwen add for contacts enter account 20190419
			{
				actionBar.setDisplayHomeAsUpEnabled(false);
            	actionBar.setHomeButtonEnabled(false);
            	actionBar.setDisplayShowTitleEnabled(true);//chengyuwen change false to true to display title 20201103
			}
			else
			//[E]wangchunyan 20190418 add to fix bug 53888	
			{
            actionBar.setDisplayHomeAsUpEnabled(deviceProvisioned);
            actionBar.setHomeButtonEnabled(deviceProvisioned);
            actionBar.setDisplayShowTitleEnabled(true);
			}
            // urovo add yuanwei for AgingTest intent 2021-03-23 begin
            //Log.d(LOG_TAG, "Starting onCreate action = " + getIntent().getAction());
            if(getIntent().getAction() != null && getIntent().getAction().equals(android.provider.Settings.ACTION_SECURITY_SETTINGS)){
                actionBar.setDisplayHomeAsUpEnabled(false);
                actionBar.setHomeButtonEnabled(false);
            }
            // urovo add yuanwei for AgingTest intent 2021-03-23 end
        }
    ...
    private void doUpdateTilesList() {
        ...
        //#ifdef VENDOR_UROVO
        //add by dengnaliang remove settings
        String hideHomePageItemsProp = SystemProperties.get("persist.sys.urv.hide.settings.homepage.items", null);
        if(!TextUtils.isEmpty(hideHomePageItemsProp)) {
            String[] hideList = hideHomePageItemsProp.split(",");
            for(int hideIdx = 0; hideIdx < hideList.length; hideIdx ++ ) {

                for(int entryIdx = 0; entryIdx < SettingsGateway.SETTINGS_FOR_RESTRICTED.length; entryIdx ++ ) {

                    boolean bContains = SettingsGateway.SETTINGS_FOR_RESTRICTED[entryIdx].contains(hideList[hideIdx]);

                    if(bContains) {
                        somethingChanged = setTileEnabled(changedList,
                            new ComponentName(packageName, SettingsGateway.SETTINGS_FOR_RESTRICTED[entryIdx]),false,
                            isAdmin) || somethingChanged;
                    }
                }
            }
