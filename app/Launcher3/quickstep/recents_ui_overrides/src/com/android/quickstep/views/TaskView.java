...
public class TaskView extends FrameLayout implements PageCallbacks, Reusable {
	...
	//zhangguangfen_20200813 add lockTasks for SQ53Q S
	mLockButton = (ImageView) findViewById(R.id.lock);
	mLockButton.setOnClickListener(new View.OnClickListener() {
		@Override
		public void onClick(View v) {
			ArrayList<String> lockNames = new ArrayList<String>();
			if(!getRecentsView().lockPackages.contains(packName)){
				getRecentsView().lockPackages.add(packName);
				mLockButton.setImageDrawable(getResources().getDrawable(
						R.drawable.lock));
			}else{
				getRecentsView().lockPackages.remove(packName);
				mLockButton.setImageDrawable(getResources().getDrawable(
						R.drawable.unlock));
			}
			lockNames = getRecentsView().lockPackages;
			String str = String.join(",",lockNames.toArray(new String[lockNames.size()]));
			Settings.System.putString(getContext().getContentResolver(),"locknames",str);
		}
	});
	//zhangguangfen_20200813 add lockTasks for SQ53Q E
    }
	...
	//zhangguangfen_20200813 add lockTasks for SQ53Q S
	packName = mTask.getTopComponent().getPackageName();
	Log.w(TAG, "packName= " + packName);
	if(getRecentsView() != null){
		if(getRecentsView().lockPackages != null){
			if(!getRecentsView().lockPackages.contains(packName)){
				mLockButton.setImageDrawable(getResources().getDrawable(
						R.drawable.unlock));
			}else{
