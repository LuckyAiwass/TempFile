...
public abstract class RecentsView<T extends BaseActivity> extends PagedView implements Insettable,
    ...
    public ArrayList<String> lockPackages = new ArrayList<String>();
    ...
    public RecentsView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //zhangguangfen_20200813 add lockTasks for SQ53Q S
        String str_names=Settings.System.getString(context.getContentResolver(),"locknames");
        if(str_names != null){
            String[] strs = str_names.split(",", 0);
            for (int i = 0, k = strs.length; i < k; i++) {
                 lockPackages.add(strs[i]);
                 }
             }
        //zhangguangfen_20200813 add lockTasks for SQ53Q E
    ...
    protected void onAllTasksRemoved() {
		...
		Intent intent = new Intent("com.android.clearButtonState");
		intent.putExtra("clear_hasTask", false);
		intent.putExtra("clear_visible", false);
    ...
    public void reset() {
        ...
        unloadVisibleTaskData();
        setCurrentPage(0);
        clear_all = false; //zhangguangfen_20200813 add lockTasks for SQ53Q
    ...
    public PendingAnimation createTaskDismissAnimation(TaskView taskView, boolean animateTaskView,
        ...
        mPendingAnimation.addEndListener((onEndListener) -> {
           if (onEndListener.isSuccess) {
			   ...
			   if (shouldRemoveTask) { //zhangguangfen_20200813 add lockTasks for SQ53Q
				   removeView(taskView);
			   }
               if (getTaskViewCount() == 0 ) {
                   removeView(mClearAllButton);
	...
	for (int i = 0; i < count; i++) {
		...
		String packName = child.getTask().getTopComponent().getPackageName();
		if (!lockPackages.contains(packName)){
	           dismissTask(child, true /*animateTaskView*/, true /*removeTask*/);
		}else{
	            dismissTask(child, false /*animateTaskView*/, false /*removeTask*/);
		}
		if(i == count -1){
			lastOne = true;
		}
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (count >0 && !clear_all) { //zhangguangfen_20200813 add lockTasks for SQ53Q
			Intent intent = new Intent("com.android.clearButtonState");
			intent.putExtra("clear_hasTask", true);
			intent.putExtra("clear_visible", true);
			mActivity.sendBroadcast(intent);
		}
		if(clear_all){ //zhangguangfen_20200813 add lockTasks for SQ53Q
			clear_all = false;
		}
