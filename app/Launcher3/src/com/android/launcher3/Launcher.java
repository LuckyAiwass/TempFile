...
import com.android.launcher3.views.CircleProgress;
import com.android.launcher3.views.BaseDragLayer;
import com.android.quickstep.views.RecentsView;
import com.android.launcher3.clean.MemInfoReader;
import com.android.launcher3.clean.CleanClass;
import android.content.pm.PackageInfo;
//gongcheng add [e]
...
public class Launcher extends BaseDraggingActivity implements LauncherExterns,
	...
	//add by gongcheng for cleaner [s]
	static RecentsView mRecentsView;
	private CircleProgress mClearButton;
	private TextView mRamSize;
	private static final int DEFAULT_MAX_VALUE = 100;	
	private static final int RAM_CHANGE_TIMER = 1000;	
	private static final int RAM_CLEANING_TIMER = 60;
	private static final String DEFAULT_RAM_VALUE = "G";
	private static final int MSG_START = 1;
	private static final int MSG_RAM_CHANGE = 2;
	private static final int MSG_CLEAN_RECENTS = 3;
	private static final int REQUEST_PERMISSION_KILL_UID = 7;
	private int addprogress;
	private int deprogress;
	private boolean cleaned = false;
	private boolean cleaning = false;
	private boolean mShowing = false;
	private long rammb = 1024 * 1024 *1024;
	private long ramsize = 01;
	private String string_freeram = null;
	public boolean hasRunningTask = false;
	public boolean isClearButtonVisible = false;
	private long mRamTotalSize = 4 * rammb;
	//add by gongcheng for cleaner [e]
...
//add by gongcheng for cleaner [s]
	...
	public void handleMessage(Message msg) {
    	switch (msg.what) {
			case MSG_START:
				if(!cleaned) {
					if(addprogress < 0) {
						CleanClass cleaner = new CleanClass(Launcher.this);
						cleaner.startclean();
						mRecentsView.dismissTasks();
						cleaned = true;
					} else {
						addprogress -= 5;
						mClearButton.setMainProgress(addprogress);
					}
				} else {
					deprogress = DEFAULT_MAX_VALUE-(int)(DEFAULT_MAX_VALUE*getRamFreeMemSize()/mRamTotalSize);
					if(addprogress > deprogress) {
						setRamText();
						addprogress -= 5;
						mClearButton.setMainProgress(addprogress > deprogress?addprogress:deprogress);
					} else {
						cleaning = false;
						setRamText();
						break;
				...
				mHandler.sendEmptyMessageDelayed(MSG_START, RAM_CLEANING_TIMER);
				break;
			case MSG_RAM_CHANGE:
				if(mShowing) {
					setRamText();
					mHandler.sendEmptyMessageDelayed(MSG_RAM_CHANGE, RAM_CHANGE_TIMER);
				}
				break;
			}
		}
	...
	private void setRamText() {
		int progress;
		progress = DEFAULT_MAX_VALUE - (int)(DEFAULT_MAX_VALUE*getRamFreeMemSize()/mRamTotalSize);
		if(string_freeram == null)
			string_freeram = getResources().getText(R.string.status_bar_recent_ram_free).toString();
		if(mRamSize != null)
			mRamSize.setText(convertStorage(getRamFreeMemSize())+" "+string_freeram+"/"+convertStorage(mRamTotalSize));
		if(mClearButton != null)
			mClearButton.setMainProgress(progress);
	}
	public static String convertStorage(long size) {
		...
		if (size >= gb) {
			int f = (int)( size % mb);
			return String.format(f ==0 ? "%.0f G" :"%.1f G", (float) size / gb);
		} else if (size >= mb) {
	...
	public void setClearButton(boolean isVerticalBarLayout, boolean isVisible , boolean hasTask) {
		if (isVisible) {
			mHotseat.setVisibility(View.GONE);
			if (hasTask) {
				mShowing = true;
				mClearButton.setVisibility(View.VISIBLE);
				mRamSize.setVisibility(View.VISIBLE);
				setRamText();
				BaseDragLayer.LayoutParams clearParams = (BaseDragLayer.LayoutParams) mClearButton.getLayoutParams();
				BaseDragLayer.LayoutParams ramParams = (BaseDragLayer.LayoutParams) mRamSize.getLayoutParams();
				if (isVerticalBarLayout) {
					clearParams.bottomMargin = 38;
					ramParams.bottomMargin = 0;
				} else {
					clearParams.bottomMargin = 240;
					ramParams.bottomMargin = 200;
				}
				mHandler.sendEmptyMessageDelayed(MSG_RAM_CHANGE, RAM_CHANGE_TIMER);
				mClearButton.setLayoutParams(clearParams);
				mRamSize.setLayoutParams(ramParams);
			}
		} else {
			mClearButton.setVisibility(View.GONE);
			mRamSize.setVisibility(View.GONE);
	...
	//urovo add by lincong for autoinstall 2020.09.19 start
	String autoinstall = Settings.System.getString(getContentResolver(),KEY_AUTO_INSTALL_APPS);
            if (autoinstall != null && autoinstall.equals("true")){
                Intent autoIntent = new Intent();
                ComponentName comp = new ComponentName("com.ubx.autoinstall","com.ubx.autoinstall.service.AutoInstallService");
                //autoIntent.setPackage("com.ubx.autoinstall");
                autoIntent.setComponent(comp);
                autoIntent.putExtra("bootStart", true);
                startService(autoIntent);
            }
        //urovo add by lincong for autoinstall 2020.09.19 end
	...
	registerReceiver(mClearButtonReceiver, new IntentFilter("com.android.clearButtonState"));//gongcheng add
    ...
    private void setupViews() {
		...
		//gongcheng add for cleaner [s]
		mRecentsView = (RecentsView)mOverviewPanel;
		mRamSize = findViewById(R.id.phone_ram);
		mClearButton = findViewById(R.id.clear_button);
		mClearButton.setOnClickListener(new OnClickListener() {
            		public void onClick(View v) { 
				hasRunningTask = false;
				ramsize = getRamFreeMemSize();
				cleaned = false;
				cleaning = true;
				mShowing = false;
				addprogress = DEFAULT_MAX_VALUE-(int)(DEFAULT_MAX_VALUE*getRamFreeMemSize()/mRamTotalSize);
				mHandler.sendEmptyMessageDelayed(MSG_START, RAM_CLEANING_TIMER);
            }
        });
		//gongcheng add for cleaner [e]
	...
	//add by gongcheng for search app [s]
	public boolean isSearchAppInstall() {
		PackageInfo packageInfo = null;
		try {
			packageInfo = this.getPackageManager().getPackageInfo(
						"com.android.quicksearchbox", 0);
		} catch (Exception e) {
			packageInfo = null;
		}
		try {
		...
		if (packageInfo == null) {
			Settings.System.putInt(getContentResolver(),"isSearchAppInstall", 0);
			return false;
		} else {
			Settings.System.putInt(getContentResolver(),"isSearchAppInstall", 1);
			return true;
		}
	}
	//add by gongcheng for search app [e]
	
	//add by chenchuanliang for check search app [s]
	public static boolean isSearchAppInstall(Context context) {
		PackageInfo packageInfo = null;
		try {
			packageInfo = context.getPackageManager().getPackageInfo("com.android.quicksearchbox", 0);
		} catch (Exception e) {
			packageInfo = null;
		}
		try {
	...
	//add by chenchuanliang for search app [e]
	...
	//add by gongcheng 20190412 for ClearAllButton [s]
	private final BroadcastReceiver mClearButtonReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            hasRunningTask = intent.getBooleanExtra("clear_hasTask", false);
	    isClearButtonVisible = intent.getBooleanExtra("clear_visible", false);
	    setClearButton(mDeviceProfile.isVerticalBarLayout(), isClearButtonVisible, hasRunningTask);
        }
    	};
	//add by gongcheng 20190412 for ClearAllButton [e]
	...
	unregisterReceiver(mClearButtonReceiver);//gongcheng add
	...
	setClearButton(mDeviceProfile.isVerticalBarLayout(), isClearButtonVisible, hasRunningTask);//gongcheng add
