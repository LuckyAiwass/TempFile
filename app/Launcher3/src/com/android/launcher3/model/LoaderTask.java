...
import android.provider.Settings; //urovo added
...
public class LoaderTask implements Runnable {
    ...
    private List<LauncherActivityInfo> loadAllApps() {
        ...
        //urovo add by lq
        List<String> hideIconLists = null;
        try{
            String hidepackageNames =  Settings.System.getString(mApp.getContext().getContentResolver(), "HideApplicationIcon");
            if (!android.text.TextUtils.isEmpty(hidepackageNames)) {
                String[] split = hidepackageNames.split(",");
                hideIconLists = java.util.Arrays.asList(split);
            }
        } catch(Exception e){
        }
        //urovo add end
		...
		//urovo add by lq 
                if(hideIconLists != null && hideIconLists.size() > 0){
                    if(hideIconLists.contains(app.getComponentName().getPackageName())) {
                        //not TODO
                    } else {
